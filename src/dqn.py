...
class DQN(object):
  ...
  def _add_placeholders(self):
    ...
    self._x = tf.placeholder(
        tf.float32, 
        [None, self._hps.dqn_input_feature_len], 
        name='x') 
    self._y = tf.placeholder(
        tf.float32,
        [None, self._hps.vocab_size], 
        name='y') 
  ...
  def _add_tf_layers(self):
    """ Based on the dqn_layers flag, it creates multiple dense layers to do
    the regression. """

    h = tf.layers.dense(
        self._x,
        units = self._hps.dqn_input_feature_len,
        activation=tf.nn.relu, 
        name='{}_input_layer'.format(self._name_variable))
    for i, layer in enumerate(self._hps.dqn_layers.split(',')):
      h = tf.layers.dense(
          h, 
          units = int(layer), 
          activation = tf.nn.relu, 
          name='{}_h_{}'.format(self._name_variable, i))

    self.advantage_layer = tf.layers.dense(
      h, 
      units=self._hps.vocab_size, 
      activation=tf.nn.softmax, 
      name='{}_advantage'.format(self._name_variable))

    if self._hps.dueling_net:
      # in dueling net, we have two extra output layers; one for value 
      # function estimation and the other for advantage estimation, we 
      # then use the difference between these two layers
      # to calculate the q-estimation
      self_layer = tf.layers.dense(
          h, 
          units = 1, 
          activation = tf.identity, 
          name='{}_value'.format(self._name_variable))
      # equation 9 in https://arxiv.org/pdf/1511.06581.pdf
      normalized_al = self.advantage_layer - tf.reshape(
          tf.reduce_mean(self.advantage_layer,axis=1),
          [-1,1]) 
  ...
  def _add_train_op(self):
    ...
    self.loss = tf.losses.mean_squared_error(
        labels = self._y, predictions = self.output)
  ...
  def _add_update_weights_op(self):
    """Updates the weight of the target network based on the current 
    network."""
    # target variables
    self.model_trainables = tf.trainable_variables(
        scope='{}_relay_network'.format(self._name_variable)) 
    self._new_trainables = [tf.placeholder(
      tf.float32, 
      None,
      name='trainables_{}'.format(i)) for i in range(len(self.model_trainables))]
    self.assign_ops = []

    # target parameters are slowly updating using: 
    # \phi_target = \tau * \phi_target + (1-\tau) * \phi_target
  ...
  def build_graph(self):
    with tf.variable_scope(
        '{}_relay_network'.format(self._name_variable)), tf.device(
        '/gpu:{}'.format(self._hps.dqn_gpu_num)):
